...
public class Block {
    ...
    private volatile boolean found = false; //boolean to keep trap if we mined the hash
    ...
    public String mineBlockAsync(int prefix, int startNonce, int endNonce) {
        ...
        if(getFound()){
            if (local_hash.startsWith(prefixString)) {
                ...
                //System.out.println("Thread " + Thread.currentThread().getName() + " finished mining + nonce "+local_nonce);
                return null;
            }
